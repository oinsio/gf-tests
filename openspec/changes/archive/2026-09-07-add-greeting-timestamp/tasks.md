...
- [x] 1.1 Thread a `java.time.Clock` parameter through `HelloWorld.run` and
...
- [x] 2.1 Update `HelloWorld.readHistory`/`HelloWorld.run` so a stored line
...
- [x] 2.2 Verify with `HelloWorldSpec` that a history file mixing
...
- [x] 3.1 Run `./gradlew test` and confirm all `HelloWorldSpec` scenarios
