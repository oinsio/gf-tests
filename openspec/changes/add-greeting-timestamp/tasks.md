...
- [ ] 1.1 Add a `java.time.Clock`-accepting overload of `appendGreeting` that
      ...
      injected clock`).
- [ ] 1.2 Update `run(...)` to accept and pass through the clock to the save
...
- [ ] 2.1 Update `readHistory`/the display logic in `run(...)` to parse a
      ...
      with its timestamp`).
- [ ] 2.2 Verify legacy (untimestamped) lines already covered by existing
...
- [ ] 3.1 Update existing specs whose assertions hard-code the old
